...
func (d *DataAnalysis) GetUserPortrait(accessToken, begin, end string) (*UserPortrait, error) {
	api := common.BaseURL + apiGetUserPortrait
	return getUserPortrait(accessToken, begin, end, api)
}

func getUserPortrait(accessToken, begin, end, api string) (*UserPortrait, error) {
	api, err := httpclient.TokenAPI(api, accessToken)
	if err != nil {
		return nil, err
	}
	...
	if err := httpclient.PostJSON(api, params, res); err != nil {
		return nil, err
	}
...
func (d *DataAnalysis) GetVisitDistribution(accessToken, begin, end string) (*VisitDistribution, error) {
	api := common.BaseURL + apiGetVisitDistribution
...
func (d *DataAnalysis) GetVisitPage(accessToken, begin, end string) (*VisitPage, error) {
	api := common.BaseURL + apiGetVisitPage
...
// begin 开始日期。格式为 yyyymmdd
// end 结束日期，限定查询1天数据，允许设置的最大值为昨日。格式为 yyyymmdd
func (d *DataAnalysis) GetDailySummary(accessToken, begin, end string) (*DailySummary, error) {
	api := common.BaseURL + apiGetDailySummary
