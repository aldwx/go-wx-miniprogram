...
func (srv *Server) handleRequest(w http.ResponseWriter, r *http.Request, isEncrpt bool, tp dataType) (interface{}, error) {
	...
	if isEncrpt { // 处理加密消息
		...
		nonce := httpclient.GetQuery(r, "nonce")
		signature := httpclient.GetQuery(r, "msg_signature")
		timestamp := httpclient.GetQuery(r, "timestamp")
...
func (srv *Server) Serve(w http.ResponseWriter, r *http.Request) error {
	...
	case "POST":
		...
		return nil
	case "GET":
		echostr := httpclient.GetQuery(r, "echostr")
		if srv.validate {
...
func isEncrypted(req *http.Request) bool {
	return httpclient.GetQuery(req, "encrypt_type") == "aes"
}
...
func (srv *Server) validateServer(req *http.Request) bool {
	nonce := httpclient.GetQuery(req, "nonce")
	signature := httpclient.GetQuery(req, "signature")
	timestamp := httpclient.GetQuery(req, "timestamp")
...
func (srv *Server) encryptMsg(message string, timestamp int64) (*EncryptedMsgRequest, error) {
	...
	//获得16位随机字符串，填充到明文之前
	nonce := httpclient.RandomString(16)
