...
// token  接口调用凭证
// id  附近地点 ID
func (p *NearbyPoi) DeleteNearbyPoi(token, id string) (*common.CommonError, error) {
	api := common.BaseURL + apiDeleteNearbyPoi
	return deleteNearbyPoi(api, token, id)
}

func deleteNearbyPoi(api, token, id string) (*common.CommonError, error) {
	url, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
...
// page  起始页id（从1开始计数）
// rows  每页展示个数（最多1000个）
func (p *NearbyPoi) GetNearbyPoiList(token string, page, rows uint) (*PositionList, error) {
	api := common.BaseURL + apiGetNearbyPoiList
...
func getNearbyPoiList(api, token string, page, rows uint) (*PositionList, error) {
	...
	err = jsoniter.Unmarshal([]byte(res.Data.RawData), &res.Data.Data)
	if err != nil {
		return nil, err
	}
...
// poiID  附近地点 ID
// status  是否展示
func (p *NearbyPoi) SetNearbyPoiShowStatus(token, poiID string, status NearbyPoiShowStatus) (*common.CommonError, error) {
	api := common.BaseURL + apiSetNearbyPoiShowStatus
	return setNearbyPoiShowStatus(api, token, poiID, status)
}

func setNearbyPoiShowStatus(api, token, poiID string, status NearbyPoiShowStatus) (*common.CommonError, error) {
	url, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
