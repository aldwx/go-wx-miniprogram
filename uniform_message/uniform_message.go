...
//
// token access_token
func (msg *UniformMsgSender) Send(token string) (*common.CommonError, error) {
	api := common.BaseURL + apiSendUniformMessage
	return msg.send(api, token)
}

func (msg *UniformMsgSender) send(api, token string) (*common.CommonError, error) {
	...
	if err := httpclient.PostJSON(api, msg, res); err != nil {
		return nil, err
	}
