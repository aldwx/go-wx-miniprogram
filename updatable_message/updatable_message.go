...
// token 接口调用凭证
func (u *UpdatableMessage) CreateActivityID(token string) (*CreateActivityIDResponse, error) {
	api := common.BaseURL + apiCreateActivityID
...
func createActivityID(api, token string) (*CreateActivityIDResponse, error) {
	...
	if err := util.getJSON(api, res); err != nil {
		return nil, err
	}
...
// Set 修改被分享的动态消息。
// accessToken 接口调用凭证
func (setter *UpdatableMsgSetter) Set(token string) (*common.CommonError, error) {
	api := common.BaseURL + apiSetUpdatableMsg
	return setter.set(api, token)
}

func (setter *UpdatableMsgSetter) set(api, token string) (*common.CommonError, error) {
	api, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
	...
	if err := httpclient.PostJSON(api, setter, res); err != nil {
		return nil, err
	}
