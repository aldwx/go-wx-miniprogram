...
func (e *Express) GetContact(accessToken, token, watBillID string) (*GetContactResponse, error) {
	api := common.BaseURL + apiGetContact
...
func getContact(api, accessToken, token, watBillID string) (*GetContactResponse, error) {
	url, err := httpclient.TokenAPI(api, accessToken)
	if err != nil {
		return nil, err
	}
...
func (previewer *ExpressTemplatePreviewer) Preview(token string) (*PreviewTemplateResponse, error) {
	api := common.BaseURL + apiPreviewTemplate
...
func (previewer *ExpressTemplatePreviewer) preview(api, token string) (*PreviewTemplateResponse, error) {
	...
	if err := httpclient.PostJSON(url, previewer, res); err != nil {
		return nil, err
	}
...
// Update 更新商户审核结果
// token 接口调用凭证
func (updater *BusinessUpdater) Update(token string) (*common.CommonError, error) {
	api := common.BaseURL + apiUpdateBusiness
	return updater.update(api, token)
}

func (updater *BusinessUpdater) update(api, token string) (*common.CommonError, error) {
	url, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
...
// Update 更新运单轨迹
// token 接口调用凭证
func (updater *ExpressPathUpdater) Update(token string) (*common.CommonError, error) {
	api := common.BaseURL + apiUpdatePath
	return updater.update(api, token)
}

func (updater *ExpressPathUpdater) update(api, token string) (*common.CommonError, error) {
	url, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
