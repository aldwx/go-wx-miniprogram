...
func TestOnAddExpressOrder(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		srv, err := common.NewServer("mock-app-id", "mock-access-token", aesKey, "mock-mch-id", "mock-api-key", false)
		if err != nil {
			t.Fatal(err)
		}

		srv.OnAddExpressOrder(func(result *common.AddExpressOrderResult) *common.AddExpressOrderReturn {
			...
			res := common.AddExpressOrderReturn{
				CommonServerReturn: common.CommonServerReturn{
	...
	res := new(common.AddExpressOrderReturn)
	...
	res = new(common.AddExpressOrderReturn)
	if err := jsoniter.NewDecoder(jsonResp.Body).Decode(res); err != nil {
		t.Error(err)
	}
...
func TestOnCancelExpressOrder(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		srv.OnCancelExpressOrder(func(result *common.CancelExpressOrderResult) *common.CancelExpressOrderReturn {
			if result.ToUserName == "" {
				t.Error("ToUserName can not be empty")
			}
			if result.FromUserName == "" {
			...
			res := common.CancelExpressOrderReturn{
				CommonServerReturn: common.CommonServerReturn{
	...
	defer xmlResp.Body.Close()
	res := new(common.CancelExpressOrderReturn)
	if err := xml.NewDecoder(xmlResp.Body).Decode(res); err != nil {
		t.Error(err)
	}
...
func TestOnCheckBusiness(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		srv.OnCheckExpressBusiness(func(result *common.CheckExpressBusinessResult) *common.CheckExpressBusinessReturn {
			if result.ToUserName == "" {
				t.Error("ToUserName can not be empty")
			}
			if result.FromUserName == "" {
			...
			res := common.CheckExpressBusinessReturn{
				CommonServerReturn: common.CommonServerReturn{
	...
	defer xmlResp.Body.Close()
	res := new(common.CheckExpressBusinessReturn)
	if err := xml.NewDecoder(xmlResp.Body).Decode(res); err != nil {
		t.Error(err)
	}
...
func TestOnGetExpressQuota(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		srv.OnGetExpressQuota(func(result *common.GetExpressQuotaResult) *common.GetExpressQuotaReturn {
			if result.ToUserName == "" {
				t.Error("ToUserName can not be empty")
			}
			if result.FromUserName == "" {
			...
			res := common.GetExpressQuotaReturn{
				CommonServerReturn: common.CommonServerReturn{
	...
	defer xmlResp.Body.Close()
	res := new(common.GetExpressQuotaReturn)
	if err := xml.NewDecoder(xmlResp.Body).Decode(res); err != nil {
		t.Error(err)
	}
...
func TestPreviewTemplate(t *testing.T) {
	...
	err := jsoniter.Unmarshal([]byte(raw), previewer)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdateBusiness(t *testing.T) {
	...
	err := jsoniter.Unmarshal([]byte(raw), updater)
	if err != nil {
		t.Fatal(err)
	}
