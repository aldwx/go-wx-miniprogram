...
func (msg CSMsgText) SendTo(openID, token string) (*common.CommonError, error) {
...
// openID 用户openID
// token 微信 access_token
func (msg CSMsgImage) SendTo(openID, token string) (*common.CommonError, error) {
...
// openID 用户openID
// token 微信 access_token
func (msg CSMsgLink) SendTo(openID, token string) (*common.CommonError, error) {
...
// openID 用户openID
// token 微信 access_token
func (msg CSMsgMPCard) SendTo(openID, token string) (*common.CommonError, error) {
...
//
// token 微信 access_token
func sendMessage(token string, params interface{}) (*common.CommonError, error) {
	api := common.BaseURL + apiSendMessage
	return doSendMessage(token, params, api)
}

func doSendMessage(token string, params interface{}, api string) (*common.CommonError, error) {
...
// openID 用户的 OpenID
// cmd 命令
func (c *CustomerMessage) SetTyping(token, openID string, cmd SetTypingCommand) (*common.CommonError, error) {
	api := common.BaseURL + apiSetTyping
	return setTyping(token, openID, cmd, api)
}

func setTyping(token, openID string, cmd SetTypingCommand, api string) (*common.CommonError, error) {
	url, err := httpclient.TokenAPI(api, token)
	if err != nil {
		return nil, err
	}
...
func UploadTempMedia(token string, mediaType TempMediaType, medianame string) (*UploadTempMediaResponse, error) {
	api := common.BaseURL + apiUploadTemplateMedia
...
func uploadTempMedia(token string, mediaType TempMediaType, medianame, api string) (*UploadTempMediaResponse, error) {
	...
	if err := httpclient.PostFormByFile(url, "media", medianame, res); err != nil {
		return nil, err
	}
...
// token 接口调用凭证
// mediaID 媒体文件 ID
func (c *CustomerMessage) GetTempMedia(token, mediaID string) (*http.Response, *common.CommonError, error) {
	api := common.BaseURL + apiGetTemplateMedia
	return getTempMedia(token, mediaID, api)
}

func getTempMedia(token, mediaID, api string) (*http.Response, *common.CommonError, error) {
	queries := httpclient.RequestQueries{
		"access_token": token,
		"media_id":     mediaID,
	}
	...
	response := new(common.CommonError)
