...
func (code *QRCode) Get(token string) (*http.Response, *common.CommonError, error) {
	api := common.BaseURL + apiGetQrCode
	return code.get(api, token)
}

func (code *QRCode) get(api, token string) (*http.Response, *common.CommonError, error) {
...
//
// token 微信access_token
func (code *UnlimitedQRCode) Get(token string) (*http.Response, *common.CommonError, error) {
	api := common.BaseURL + apiGetUnlimitedQRCode
	return code.get(api, token)
}

func (code *UnlimitedQRCode) get(api, token string) (*http.Response, *common.CommonError, error) {
	return qrCodeRequest(api, token, code)
}
...
//
// token 微信access_token
func (creator *QRCodeCreator) Create(token string) (*http.Response, *common.CommonError, error) {
	api := common.BaseURL + apiCreateQRCode
	return creator.create(api, token)
}

func (creator *QRCodeCreator) create(api, token string) (*http.Response, *common.CommonError, error) {
	return qrCodeRequest(api, token, creator)
}

// 向微信服务器获取二维码
// 返回 HTTP 请求实例
func qrCodeRequest(api, token string, params interface{}) (*http.Response, *common.CommonError, error) {
	...
	res, err := httpclient.PostJSONWithBody2(url, params)
	if err != nil {
		return nil, nil, err
	}
